...
fun createUfo(maxWidth: Int, maxHeight: Int) : Ufo =
    Ufo((0.. maxWidth - UFO_WIDTH).random(), (0..maxHeight-UFO_HEIGHT).random(),
            (-2..2).random(),2, UFO_WIDTH ,UFO_HEIGHT)
...
fun draw(cv : Canvas, u: Ufo){
    ...
    cv.drawRect(u.x , u.y  + blockSize, u.width, blockSize * 3, GREEN)
    cv.drawRect(u.x + blockSize * 2, u.y + blockSize * 4, blockSize *6, blockSize, GREEN)
}